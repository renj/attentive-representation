        ...
        for _j in range(self.rl_batch):
            ...
            for _i in range(self.nlayers):
                ...
                if self.rl:
                    if self.state == 'eval':
                        ...
                        if self.rl_torch:
                            ...
                            for i, l in enumerate(lens):
                                ...
                                sample = np.random.choice(range(slen), p=_w)
                                '''
                                _w = A[i].data.squeeze().cpu().double()
                                sample = list(torch.utils.data.sampler.WeightedRandomSampler(_w, 1))[0]
                                select = np.random.choice(["sample", "random"], p=[1.0 - self.epsilon, self.epsilon])
                                if select == 'sample':
                                    _w = A[i].data.squeeze().cpu().double()
                    ...
                    _Ms = []
                    #a = a.view(-1).cpu()
                    for i, _a in enumerate(a.view(-1).cpu().data):
                        _Ms.append(H[i][_a].view(1, self.u))
                    M = torch.cat(_Ms)
                else:
                    A = A.view(batch, 1, slen)
                ...
                if self.pause:
                    if self.isolate_P:
                        if self.H_h:
                            if self.q_P:
                                _q = Variable(q.data)
                                _tmp = torch.cat([_q, Variable(H_sum.data)], 1)
                                P = F.softmax(self.P(_tmp))
                            else:
                                _Ms = Variable(_Ms.data)
                                _tmp = torch.cat([_Ms, Variable(H_sum.data)], 1)
                                P = F.softmax(self.P(_tmp))
                        else:
                            if self.q_P:
            ...
            if self.pause is False and self.hinge is False:
                Ds += [D]
            #out = F.softmax(D)
            #out1s.append(self.dropout(D))
            if self.aoa:
